...
function statusSelect(type, row, options) { return `<select data-admin-type="${type}" data-admin-id="${row.id}">${options.map(option => `<option value="${option}"${row.status === option ? ' selected' : ''}>${escapeHtml(option)}</option>`).join('')}</select>`; }
